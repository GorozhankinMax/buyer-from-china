...
const services = [
  ...
  { icon: "02", title: "Проверка качества", text: "Контролируем образцы, производство, упаковку и соответствие вашим требованиям." },
  { icon: "03", title: "Сопровождение сделки", text: "Берем на себя переговоры, согласование условий и сопровождение оплаты." },
  { icon: "04", title: "Логистика под ключ", text: "Организуем выкуп, консолидацию, документы и доставку до вашего города." },
];

const cases = [
  { title: "Товары для маркетплейсов", result: "32%", text: "Снизили закупочную стоимость партии бытовых товаров.", term: "Срок поставки: 12 дней" },
  { title: "Электроника", result: "18 дней", text: "Собрали поставку из нескольких фабрик в одну отгрузку.", term: "Полный цикл от поиска до получения" },
  { title: "Упаковка и расходники", result: "3X", text: "Увеличили объем закупки без просадки по качеству.", term: "Рост объема за 2 месяца работы" },
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const getRevealGroup = (element) => {
      ...
      if (explicitGroup) return Array.from(explicitGroup.querySelectorAll("[data-reveal]"));

      let current = element.parentElement;
      while (current && current !== document.body) {
        const groupItems = Array.from(current.querySelectorAll("[data-reveal]"));
        if (groupItems.length > 1 && groupItems.includes(element)) return groupItems;
    ...
    const getDelayValue = (element) => {
      if (!element.dataset.baseDelay) element.dataset.baseDelay = element.style.transitionDelay || "0ms";
  ...
  useEffect(() => {
    document.body.style.overflow =
      isCalculatorModalOpen || isContactModalOpen || isPrivacyModalOpen || isMobileMenuOpen ? "hidden" : "";
    return () => {
      document.body.style.overflow = "";
    };
  ...
  useEffect(() => {
    if (!isCalculatorModalOpen) setIsCurrencyOpen(false);
  }, [isCalculatorModalOpen]);

  useEffect(() => {
    if (!isContactModalOpen) setIsContactChannelOpen(false);
  }, [isContactModalOpen]);
  ...
  const handleCalculatorSubmit = async (event) => {
    ...
    setCalculatorSubmitState({ status: "submitting", message: "Отправляем расчет..." });
    ...
    try {
      ...
      setCalculatorSubmitState({ status: "success", message: "Заявка на расчет отправлена." });
    } catch (error) {
      setCalculatorSubmitState({ status: "error", message: error.message || "Не удалось отправить заявку." });
    }
  };
  ...
  const handleContactSubmit = async (event) => {
    ...
    const form = event.currentTarget;
    const formData = new FormData(form);
    setContactSubmitState({ status: "submitting", message: "Отправляем заявку..." });
    ...
    try {
      ...
      setContactSubmitState({ status: "success", message: "Заявка отправлена." });
    } catch (error) {
      setContactSubmitState({ status: "error", message: error.message || "Не удалось отправить заявку." });
    }
  };
    ...
    <div className="page-shell">
        ...
        <div className="mobile-menu" aria-modal="true" role="dialog" aria-label="Мобильное меню">
          ...
          <div className="mobile-menu__panel">
            ...
            <nav className="mobile-menu__nav" aria-label="Мобильная навигация">
              {heroNav.map((item) => (
                <a className="glass-button glass-button--accent mobile-menu__nav-button" href={item.href} key={item.label} onClick={() => setIsMobileMenuOpen(false)}>
        ...
        <section className="section" aria-labelledby="services-title" id="services">
          <div className="section__container">
            <div className="section-heading reveal-up" data-reveal>
              ...
              <h2 className="section-heading__title" id="services-title">Закрываем весь цикл закупки из Китая</h2>
            </div>
        ...
        <section className="section" aria-labelledby="cases-title" id="cases-archive">
          <div className="section__container">
            <div className="section-heading reveal-up" data-reveal>
              ...
              <h2 className="section-heading__title" id="cases-title">Несколько результатов из реальной работы</h2>
            </div>
        ...
        <section className="section" aria-labelledby="cta-title" id="cta">
          <div className="section__container">
            <div className="glass-card cta-card reveal-up" data-reveal>
              ...
              <h2 className="section-heading__title cta-card__title" id="cta-title">Если нужна поставка без хаоса, начнем с вашей задачи</h2>
              <p className="cta-card__text">Расскажите, что хотите закупать, а мы предложим понятный маршрут: поставщик, цена, контроль качества и доставка.</p>
      ...
      {isCalculatorModalOpen ? (
        <div className="calculator-modal" aria-modal="true" role="dialog" aria-labelledby="calculator-request-title">
          ...
          <div className="calculator-modal__card">
            <button className="calculator-modal__close" type="button" aria-label="Закрыть окно" onClick={() => setIsCalculatorModalOpen(false)}>×</button>
            <h2 className="calculator-modal__title" id="calculator-request-title">Расскажите о товаре, и мы подготовим расчет</h2>
            <form className="calculator-form" onSubmit={handleCalculatorSubmit}>
              ...
              <label className="calculator-field"><span>Категория товара</span><input type="text" name="productCategory" placeholder="Например, электроника" /></label>
              <label className="calculator-field"><span>Вес</span><div className="calculator-field__with-unit"><input type="text" name="weight" placeholder="Например, 120" /><span className="calculator-field__unit">кг</span></div></label>
              <label className="calculator-field"><span>Объем</span><div className="calculator-field__with-unit"><input type="text" name="volume" placeholder="Например, 0.8" /><span className="calculator-field__unit">м3</span></div></label>
              <label className="calculator-field">
                ...
                <div className="calculator-field__compound">
                  ...
                  <div className={`calculator-select calculator-select--compact ${isCurrencyOpen ? "calculator-select--open" : ""}`}>
                    ...
                    <button className="calculator-select__trigger calculator-select__trigger--compact" type="button" aria-haspopup="listbox" aria-expanded={isCurrencyOpen} onClick={() => setIsCurrencyOpen((current) => !current)}>
                      <span className="calculator-select__value calculator-select__value--filled">{currencyOptions.find((item) => item.value === selectedCurrency)?.label ?? "USD"}</span>
                      <span className="calculator-select__icon" aria-hidden="true"><svg viewBox="0 0 20 20"><path d="M5 7.5 10 12.5 15 7.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg></span>
                    </button>
                    <div className="calculator-select__menu calculator-select__menu--compact" role="listbox" aria-label="Валюта">
                      {currencyOptions.map((item) => (
                        <button className={`calculator-select__option ${selectedCurrency === item.value ? "calculator-select__option--selected" : ""}`} key={item.value} type="button" role="option" aria-selected={selectedCurrency === item.value} onClick={() => { setSelectedCurrency(item.value); setIsCurrencyOpen(false); }}>
                          {item.label}
                        </button>
                      ))}
                    </div>
              ...
              <label className="calculator-field calculator-field--full"><span>Отправить расчет на</span><input type="text" name="contactForEstimate" placeholder="Телефон или почта" /></label>
              <div className="calculator-consent calculator-field--full">
                <label className="calculator-consent__label" htmlFor="estimate-privacy-consent">
                  ...
                  <span>Я соглашаюсь <span className="calculator-consent__inline-link" role="button" tabIndex={0} onClick={() => setIsPrivacyModalOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setIsPrivacyModalOpen(true); } }}>с политикой конфиденциальности и обработкой персональных данных</span>.</span>
                </label>
              </div>
              <div className="calculator-form__actions calculator-field--full">
                <button className="glass-button glass-button--accent calculator-form__submit" type="submit" disabled={calculatorSubmitState.status === "submitting"}><span>{calculatorSubmitState.status === "submitting" ? "Отправляем..." : "Отправить расчет"}</span></button>
                {calculatorSubmitState.message ? <p className={`calculator-form__status calculator-form__status--${calculatorSubmitState.status}`}>{calculatorSubmitState.message}</p> : null}
              </div>
            </form>
          </div>
        </div>
      ...
      {isContactModalOpen ? (
        <div className="calculator-modal" aria-modal="true" role="dialog" aria-labelledby="calculator-title">
          ...
          <div className="calculator-modal__card">
            <button className="calculator-modal__close" type="button" aria-label="Закрыть окно" onClick={() => setIsContactModalOpen(false)}>×</button>
            <h2 className="calculator-modal__title" id="calculator-title">Укажите свои контакты и мы с вами свяжемся</h2>
            <form className="calculator-form" onSubmit={handleContactSubmit}>
              <input className="calculator-form__honeypot" type="text" name="website" tabIndex="-1" autoComplete="off" />
              <label className="calculator-field"><span>Телефон</span><input type="tel" name="phone" placeholder="+7 (___) ___-__-__" /></label>
              <label className="calculator-field"><span>Почта</span><input type="email" name="email" placeholder="example@mail.com" /></label>
              <label className="calculator-field"><span>Имя</span><input type="text" name="name" placeholder="Как к вам обращаться" /></label>
              <label className="calculator-field">
                ...
                <div className={`calculator-select ${isContactChannelOpen ? "calculator-select--open" : ""}`}>
                  ...
                  <button className="calculator-select__trigger" type="button" aria-haspopup="listbox" aria-expanded={isContactChannelOpen} onClick={() => setIsContactChannelOpen((current) => !current)}>
                    <span className={`calculator-select__value ${selectedContactChannel ? "calculator-select__value--filled" : ""}`}>{contactChannels.find((item) => item.value === selectedContactChannel)?.label ?? "Выберите вариант"}</span>
                    <span className="calculator-select__icon" aria-hidden="true"><svg viewBox="0 0 20 20"><path d="M5 7.5 10 12.5 15 7.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg></span>
                  </button>
                  <div className="calculator-select__menu" role="listbox" aria-label="Предпочтительный канал связи">
                    {contactChannels.map((item) => (
                      <button className={`calculator-select__option ${selectedContactChannel === item.value ? "calculator-select__option--selected" : ""}`} key={item.value} type="button" role="option" aria-selected={selectedContactChannel === item.value} onClick={() => { setSelectedContactChannel(item.value); setIsContactChannelOpen(false); }}>
                        {item.label}
                      </button>
                    ))}
                  </div>
              ...
              <div className="calculator-form__actions calculator-field--full">
                <button className="glass-button glass-button--accent calculator-form__submit" type="submit" disabled={contactSubmitState.status === "submitting"}><span>{contactSubmitState.status === "submitting" ? "Отправляем..." : "Отправить заявку"}</span></button>
                {contactSubmitState.message ? <p className={`calculator-form__status calculator-form__status--${contactSubmitState.status}`}>{contactSubmitState.message}</p> : null}
              </div>
            </form>
          </div>
        </div>
      ...
      <footer className="footer">
        <div className="section__container footer__inner">
          ...
          <div className="footer__contacts reveal-up" data-reveal style={{ transitionDelay: "120ms" }}>
            <a className="footer__contact-link" href="tel:+79040245342">+79040245342</a>
            <a className="footer__contact-link" href="https://t.me/maksgorozhankin" target="_blank" rel="noreferrer">Телеграм</a>
            <a className="footer__contact-link" href="https://vk.com/maknaggets_77" target="_blank" rel="noreferrer">Вконтакте</a>
            <span className="footer__contact-meta">График 24/7</span>
          </div>
          <div className="footer__bottom">
            <button className="footer__policy" type="button" onClick={() => setIsPrivacyModalOpen(true)}>Политика конфиденциальности</button>
          </div>
        </div>
      </footer>
    </div>
